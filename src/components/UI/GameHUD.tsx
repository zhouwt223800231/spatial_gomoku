...
export function GameHUD({ onConfirm, onCancel, onToggleDrawer }: GameHUDProps) {
  const {
  ...
  return (
    <>
      ...
      {/* Mobile info drawer button (hidden on desktop) */}
      <div className="col-start-1 row-start-2 md:hidden pointer-events-auto">
        <button onClick={onToggleDrawer} className="glass-button text-xs px-3 py-2">📊 信息</button>
      </div>
      ...
      {/* Confirm / Cancel overlay: centered on desktop, bottom sheet on mobile */}
      {ghostPosition && (
        <div className="absolute inset-x-3 bottom-20 md:inset-x-auto md:bottom-24 md:left-1/2 md:-translate-x-1/2 pointer-events-auto z-10">
          <div className="glass-panel px-4 md:px-5 py-3 flex items-center justify-between md:justify-center gap-3 md:gap-4">
            <span className="mono-num text-[12px] text-white/70">
              <span className="hidden md:inline">Place at </span>
              <span className="accent-glow">{fmt(ghostPosition)}</span>
            </span>
            <div className="flex gap-2">
              <button onClick={onConfirm} className="glass-button--primary px-4 py-1.5 text-sm">✓ 落子</button>
              <button onClick={onCancel} className="glass-button px-4 py-1.5 text-sm">✕ 取消</button>
            </div>
          </div>
        </div>
      )}
